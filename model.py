...
from layers.LMdecoder import CustomDecoder
...
class IeGenerator(BaseJointRE):
    def __init__(self, classes_to_id, rel_to_id, max_width, num_prompts=5,
        ...
        llama_model_name = "weights/models--minlik--chinese-llama-7b-merged/snapshots/1ca4d87576f1fef4d44a949fb65bbe6b96675872"
        # self.decoder2 = CustomDecoder(llama_model_name)
    ...
    def compute_token_embeddings(self, x):
        ...
        # out['embeddings'] of shape (batch, seq_len, hidden_size)---------------- H
        queries, embed = out['queries'], out['embeddings']

        # lstm, 上下文建模, 结合整个序列的信息
        ...
        try:
            for index, el in enumerate(true_graph):
                if isinstance(el, tuple):
                    s, e, l = el
                    pos = (s, e - s, self.classes_to_id[l] - 1)  # 起始为s，长度为e-s，类别为classes_to_id[l] - 1的span的位置
                    rev = np.ravel_multi_index(pos, (L, K, C)) # 将三维索引 pos 映射为一维索引，适用于形状为 (L, K, C) 的三维数组
                    label_revel.append(rev)
                    current_rep = representation[s, e - s, self.classes_to_id[l] - 1] # 获取pos位置的span嵌入向量
    ...
    def forward(self, x):
        ...
        # all_output_transformer = self.decoder2(all_trans_input, x["graph"], memory, cache["memory_pad_mask"])
    ...
    def compute_loss(self, all_out_emb, all_output_transformer, all_labels, out_mask):
        ...
        # mask the scores(对 all_scores 张量的无效位置进行遮蔽)
