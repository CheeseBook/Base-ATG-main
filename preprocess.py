...
class GraphIEData(Dataset):
    ...
    def create_seq(self, idx):
        ...
        dict_rel = {}
        # 定义了一个空的字典 dict_rel，用于存储实体对之间的关系。
        for i, j, k in rel:
        ...
        # 添加一个 "stop_entity" 标记，用于分隔实体和关系部分。
    ...
    def __getitem__(self, idx):
        if self.type == "train":
            # 根据最大样本数量 max_num_samples 随机添加采样样本
            # add_samples 方法会使用 create_seq 方法生成基础序列，并随机抽取一定数量的样本来组合和扩展 seq 序列。
        ...
        # 调用 shuffle_entities_relations 方法，对实体和关系的顺序进行随机调整。
