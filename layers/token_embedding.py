    ...
    def get_embeddings(self, sentences, queries=None):
        ...
        with gradient_context:
            ...
            if queries is not None:
                ...
                # 在第一个维度（batch 维度）扩展 queries；
                # 然后将扩展后的 queries 沿着第一个维度重复 N 次，最终得到形状为 (N, L, D) 的张量。
                # queries(batch_size, num_queries, embedding_size)
                ...
                # 取 sequence_length 维度的前 num_queries 个标记的隐藏状态
                # 取 sequence_length 维度中从第 num_queries 开始的剩余部分
            ...
            # 对 sentence_hidden_states 中每个样本的隐藏状态进行动态裁剪，保留序列中前 subtoken_length + 1 个标记的隐藏状态。
...
#     # model = TokenRep(num_queries=3, model_name="/gpfswork/rech/pds/upa43yu/models/scibert_cased")
#     model = TokenRep(num_queries=3, model_name="/home/zhuchao/models/scibert_cased")
#
