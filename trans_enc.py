...
"""
生成了一个上三角掩码矩阵，用于在自回归模式下的掩码操作。这个掩码矩阵使得序列的第t个时间步无法看到之后的时间步，防止未来信息泄露。
"""
...
class TransDec(nn.Module):
    ...
    def forward(self, x, seq_graph=None, memory=None, memory_pad_mask=None):
        ...
        if self.add_pos_emb and seq_graph is not None:
            ...
            x[:, num_queries:, :] = self.pos_emb(x[:, num_queries:, :], seq_graph) # 跳过前 num_queries 个元素，只处理剩余部分的序列

        # mask: (seq_len, seq_len), 因果掩码, 实现自回归
